...
USISR
*/
unsigned char unitUSIDR;
unsigned char unitUSICR;
unsigned char unitUSISR;
...
void USI_TWI_Master_StoreDefaultRegs()
{
    unitUSIDR = USIDR;
    unitUSICR = USICR;
    unitUSISR = USISR;
...
void USI_TWI_Master_ReStoreDefaultRegs()
{
    USIDR = unitUSIDR;
    USICR = unitUSICR;
    USISR = unitUSISR;
...
void USI_TWI_Master_Initialise( void )
{
  ...
  USI_TWI_Master_StoreDefaultRegs();
  ...
  USI_TWI_state.errorState = USI_TWI_OK; // Default state

}

/*---------------------------------------------------------------
 USI TWI single master un-initialization function
---------------------------------------------------------------*/

void USI_TWI_Master_UnInitialise( void )
{
    ...
    USI_TWI_Master_ReStoreDefaultRegs();
}
...
//                (0x0<<USICNT0);                                     // and reset counter.
    ...
    USI_TWI_state.errorState = USI_TWI_NO_ACK_ON_DATA; // Default state
    // test
...
/* Release SCL to ensure that (repeated) Start can be performed */
  ...
  while( !(PORT_USI & (1<<PORT_USI_SCL)) );          // Verify that SCL becomes high.
  //_delay_us(T2_TWI);
  _delay_ms(1000);

/* Generate Start Condition */
  PORT_USI &= ~(1<<PORT_USI_SDA);                    // Force SDA LOW.
  //_delay_us(T2_TWI);
  _delay_ms(1000);
  ...
  USI_TWI_state.errorState = USI_TWI_OK;
#endif
  return (TRUE);
}
/*---------------------------------------------------------------
